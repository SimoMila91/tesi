...
{
	/* Shared L1: 16752 bytes, L2 buffer: 0 bytes */
	/* Local variables used by this kernel */
	AT_L2_EVENT DmaR_Evt1;
	AT_L2_EVENT DmaW_Evt1;
	KerResizeBilinear_ArgT S_KerArg0, *KerArg0 = &S_KerArg0;
	...
	Ker Arg: Out, Tiled Space: Tile0
		...
		KerArgItSpace: 20 logical tiles, 20 physical tiles
			Total Size: 76800 [Tile0, 20:[320x12, 18:320x12, 320x12], 1]
		KerArgItSpace (User Kernel Iter Order):
			[Tile0, 20:[320x12, 18:320x12, 320x12], 1]
		Tile0: [0, 3840, 3840], Tile1: [3840, 3840, 3840], Tile2; [7680, 3840, 3840]
	Ker Arg: In, Tiled Space: Tile0
		Min Pipe Depth: 0, Max Pipe Depth: 1
		KerArgItSpace: 20 logical tiles, 20 physical tiles
			Total Size: 79056 [Tile0, 20:[324x14, 18:324x14, 324x14], 1]
		KerArgItSpace (User Kernel Iter Order):
			[Tile0, 20:[324x14, 18:324x14, 324x14], 1]
		Tile0: [0, 4536, 4536], Tile1: [3888, 4536, 4536], Tile2; [7776, 4536, 4536]
	======================== End Ker Arg Iter Spaces =========================================*/
	/*=========================== Call Kernel, Invariant assignment =====================*/
	KerArg0->Win = (unsigned int) (324);
	KerArg0->Hin = (unsigned int) (244);
	KerArg0->Wout = (unsigned int) (320);
	KerArg0->Hout = (unsigned int) (240);
	KerArg0->HTileOut = (unsigned int) (12);
	/*================================= Read Tiles Prolog ===============================*/
	_C_Out=0; _SC_Out=3840;
	_SP_Out=0;
	AT_L2_COPY(0, ((AT_L2_EXT_ADDR_TYPE) In+0), ((AT_L2_INT_ADDR_TYPE) FaceDet_L1_Memory+0+0), 4536, 0, &DmaR_Evt1);
	_N_In=0;
	/*============================= End Read Tiles Prolog ===============================*/
	for (T0Ind=0; T0Ind<20; T0Ind++, T0Ind_Total++) { /* Iteration on Tile0 */
		...
		_SN_In = 0;
		if (!(T0Ind_Last)) {
			_N_In = _N_In + 0; _Off_In = (((66355*((T0Ind)+1)*12)>>16)*324); _SN_In = (4536); 
		}
		/*============================= End Prepare Tiles ===================================*/
		/*================================= Read Tiles ======================================*/
		AT_L2_WAIT(0, &DmaR_Evt1); /* Wait previous DMA read In */
		if (_SN_In) {
			AT_L2_COPY(0, ((AT_L2_EXT_ADDR_TYPE) In+_N_In+_Off_In), ((AT_L2_INT_ADDR_TYPE) FaceDet_L1_Memory+0+4536*((T0Ind_Total+1)%2)),
					_SN_In, 0, &DmaR_Evt1);
		}
		/*============================= End Read Tiles ======================================*/
		/*====================== Call Kernel LOC_LOOP =========================*/
		KerArg0->In = (unsigned char * __restrict__) (FaceDet_L1_Memory+0+4536*((T0Ind_Total)%2));
		KerArg0->Out = (unsigned char * __restrict__) (FaceDet_L1_Memory+9072+3840*((T0Ind_Total)%2));
		KerArg0->FirstLineIndex = (unsigned int) ((66355*(T0Ind)*12)>>16);
		AT_FORK(gap_ncore(), (void *) KerResizeBilinear, (void *) KerArg0);
		__CALL(KerResizeBilinear, KerArg0);
		/*================================= Write Tiles =====================================*/
		if (_SP_Out) AT_L2_WAIT(0, &DmaW_Evt1); /* Wait previous DMA write Out */
		AT_L2_COPY(0, ((AT_L2_EXT_ADDR_TYPE) Out+_C_Out), ((AT_L2_INT_ADDR_TYPE) FaceDet_L1_Memory+9072+3840*((T0Ind_Total)%2)),
				_SC_Out, 1, &DmaW_Evt1);
		/*============================= End Write Tiles =====================================*/
		/*================================= Update Arg Pipeline =============================*/
		_SP_Out = _SC_Out;
		...
		_SC_Out = 0;
		if (!(T0Ind_Last)) {
			_C_Out = _C_Out + (3840); _SC_Out = (3840); 
		}
		/*============================= End Prepare Tiles ===================================*/
	} /* End iteration on Tile0 */
	/*================================ Write Tiles Epilog ===============================*/
