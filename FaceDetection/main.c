...
void facedetection_task(void *parameters)
{
  ...
  while (1 && (NB_FRAMES == -1 || nb_frames < NB_FRAMES))
  {
    ...
    if (wifiClientConnected == 1 && ClusterCall.num_reponse > 0)
    {
      char msg[128];
      int len = snprintf(msg, sizeof(msg),
                         "{\"drone_id\":\"drone-%d\",\"object\":\"face\",\"count\":%d}\n",
                         MY_DRONE_ID,
                         ClusterCall.num_reponse);
      memcpy(txp.data, msg, len);
      txp.dataLength = len;
      cpxSendPacketBlocking(&txp);
      cpxPrintToConsole(LOG_TO_CRTP, "Sent alert packet: %s", msg);
      /**
...
{
  cpxPrintToConsole(LOG_TO_CRTP, "Got packet from GAP8 (%u)\n", cpxRx->data[0]);
}
...
void rx_client_msg(void *parameters)
{
  while (1)
  {
    cpxReceivePacketBlocking(CPX_F_APP, &rxp2);
    uint16_t counterInStm = rxp2.data[0];
    cpxPrintToConsole(LOG_TO_CRTP, "Received packet: %s\n", counterInStm);
  }
}
static void cpxPacketCallback(const CPXPacket_t *cpxRx);
...
void start_example(void)
{
  ...
  cpxPrintToConsole(LOG_TO_CRTP, "-- WiFi Face Detection example --\n");
